...
sub get_keymaps
{	
	my ($err, @keymaps) = find_files_deep($keymap_directory, $keymap_extension);
	return () if($err);
...
sub get_fontmaps 
{
	my ($err, @fontmaps) = find_files_deep($fontmap_directory, $fontmap_extension);
	return () if($err);
...
sub get_mountpoints
{
	return ('bios', 'boot', 'swap', 'root', 'home', 'usr', 'var', 'dev', 'sys');
}
...
sub generate_installer
{
    ...
    if($use_partitioning) {    
        ...
        foreach(@partition_table) {
            ...
            if(defined($last_mountpoint)) {
                ...
                emit_line($inst, "parted $install_disk unit MiB mkpart primary 1 \$$mountpoint");
            }        
    ...
    foreach(@partition_table) {
        ...
        given($mountpoint) {
            when('bios') {                
                emit_line($inst, "parted $install_disk set $partition_number bios_grub on");                                            
            }
            when('boot') {
                emit_line($inst, "parted $install_disk set $partition_number boot on");
    ...
    while(my $line = <$in>) {
        ...
        if ($line =~ /^#*[a-z]{2,3}_/) {            
            foreach(@locales) {                
                if(index($line, $_) != -1 or index($line, $locale_lang) != -1) {
                    $found = 1;
                    $line =~ s/^[#\s]+//;                
                    print $out $line;
                    last;
    ...
    if(defined($hostname)) {
        ...
        emit_line($inst, "echo \"127.0.0.1    localhost.localdomain   localhost   $hostname\" > /etc/hosts");
        emit_line($inst, "echo \"::1          localhost.localdomain   localhost   $hostname\" >> /etc/hosts");
    ...
    while(my $line = <$in>) {        
        ...
        elsif($line =~ /^[#\s]*address=/ and defined $interface and $static_ip) {
            print $out "address=$ip\n";            
        }
        elsif($line =~ /^[#\s]*netmask=/ and defined $interface and $static_ip) {
            print $out "netmask=$netmask\n";            
        }
        elsif($line =~ /^[#\s]*gateway=/ and defined $interface and $static_ip) {
