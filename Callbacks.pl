...
sub MP_focus
{
    ...
    $cui->leave_curses();
    system("clear && $partitioning_scheme /dev/" . get_disk());
    $cui->reset_curses();
...
sub MP_nav_add
{
    ...
    add_partition_table_entry('/dev/' . $partition . ':' . $mountpoint . ':' . $filesystem);    
...
sub CS_nav_continue
{
    ...
    my ($timezonelist, $localelist, $localelist_lang, $localelist_time, $localtimecb) =
        ($win->getobj('timezonelist'), $win->getobj('localelist'), $win->getobj('localelist_lang'),
         $win->getobj('localelist_time'), $win->getobj('localtimecb'));
...
sub CNET_nav_continue
{
    ...
    if(defined($interface))
    {
        ...
        if($static_ip)
        {
            ...
            unless(length $ip)
            {
                $info->text("You must choose a IP address for static interface $interface");
                return;
            }
            set_ip($ip);
            ...
            {
                $info->text("You must choose a domain for static interface $interface");
                return;
            }
            set_domain($domain);
            ...
            {
                $info->text("You must choose a netmask for static interface $interface");
                return;
            }
            set_netmask($netmask);
            ...
            {
                $info->text("You must choose a gateway for static interface $interface");
                return;
            }
            set_gateway($gateway);
        }
...
You may quit and install Arch linux with the following command: ./" . get_install_script());
    }
}
